...
/** @typedef {Date | TimestampLike | number | string | null | undefined} DateInput */
...
export function toDate(value) {
  ...
  if (value && typeof value === 'object') {
    ...
    if (typeof value.seconds === 'number' && typeof value.nanoseconds === 'number') {
      pair = [value.seconds, value.nanoseconds];
    } else if (typeof value._seconds === 'number' && typeof value._nanoseconds === 'number') {
      pair = [value._seconds, value._nanoseconds];
    }
    if (pair) {
      const d = new Date(pair[0] * 1000 + Math.floor(pair[1] / 1e6));
      return isNaN(d.getTime()) ? null : d;
    }
  }
