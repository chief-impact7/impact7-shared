...
test('배치표 합계: 이동 71+payments 11+학생 진로 1+골든 답안 1+운영 지침 1, 전역 41+진로 공공자료 3, 교집합 없음', () => {
  assert.equal(ACADEMY_SCOPED_COLLECTIONS.size, 87);
  assert.equal(ACADEMY_SCOPED_COLLECTIONS.has('golden_answers'), true);
  assert.equal(ACADEMY_SCOPED_COLLECTIONS.has('assistant_directives'), true);
