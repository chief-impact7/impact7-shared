...
function componentSettingsDefaults(config) {
  ...
  return Object.freeze({
    ...
    kakaoChannel: Object.freeze({
      label: academy.formContact.channelLabel,
      url: academy.formContact.channelUrl,
    }),
    footer: Object.freeze({
      text: academy.brandName,
      linkLabel: academy.formContact.inquiryLabel,
      linkUrl: academy.formContact.inquiryUrl,
    }),
  });
}

export const COMPONENT_SETTINGS_DEFAULTS = componentSettingsDefaults();
...
export function normalizeComponentSettings(value, cap, config) {
  const clamp = typeof cap === "function"
    ? cap
    : (text) => String(text === null || text === undefined ? "" : text);
  const source = value && typeof value === "object" ? value : {};
  const defaults = componentSettingsDefaults(config);
  return Object.fromEntries(Object.entries(SETTINGS_LIMITS).map(([group, limits]) => [
    ...
    Object.fromEntries(Object.keys(limits).map((key) => {
      ...
      const chosen = typeof raw === "string" && raw.trim() !== "" ? raw : defaults[group][key];
