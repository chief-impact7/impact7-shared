...
| `applyNaesinFreeDerivation` | fn | `(current, { classSettings, dateStr, resolveNaesinCsKey, enrollmentCode?, allEnrollments? }) → enrollment[]` — 내신/자유학기 활성 시 정규를 치환한 배열. 학생 개별 override(`naesin_days`·`naesin_schedule` / `free_days`·`free_schedule`)가 반 기본 요일·시간을 덮는다. `allEnrollments`(날짜 필터 전 전체 배열)를 주면 같은 계정에 반 설정 기간과 겹치는 명시 조각(종료된 것 포함)이 있을 때 파생하지 않아, 반 기간 확장으로 구체화 학생이 반 전체 요일로 부활하는 것을 막는다. 생략 시 `current`만 본다 |
| `deriveActiveNaesinEnrollment` | fn | `(current, { classSettings, dateStr, resolveNaesinCsKey, allEnrollments? }) → enrollment\|null` — 활성 내신 enrollment(명시/파생) 또는 null. 명시 내신·자유학기는 `start_date`·`end_date` 모두 유효할 때만 인정(end 없는 조각은 레거시로 무시, 파생 fallback). `allEnrollments`가 있으면 반 설정 기간과 겹치는 같은 계정의 명시 내신 조각(종료 포함)이 있을 때 파생하지 않는다. 아래 predicate와 applyNaesinFreeDerivation의 SSoT |
| `isNaesinActiveAt` | fn | `(current, { classSettings, dateStr, resolveNaesinCsKey, allEnrollments? }) → boolean` — 기준일 내신기간 활성 여부. 내신 active 판정은 로컬 재구현 말고 이 함수 사용(current는 호출자가 날짜 필터한 활성 enrollment 배열) |
